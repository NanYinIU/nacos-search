...
- **Version**: sourced from `build.gradle.kts` (currently `1.3.9`) — bump it there and update the `pluginDescription` / `changeNotes` blocks in `build.gradle.kts` (`patchPluginXml`) when releasing. Those are the single source; `META-INF/plugin.xml` does not duplicate them.
...
`NacosSettings` is a **master-detail, multi-server** model: a `servers: MutableList<NacosServerConfig>` plus an `activeServerId`. The flat legacy fields on `NacosSettings` (`serverUrl`, `username`, `namespace`, `authMode`, …) are **deserialization inputs only** — ADR-0049 makes them, and the legacy server list beside them, values migrated once and never written again. **No service reads them**; the only reads left are inside `NacosSettings.kt` itself (legacy-migration detection, `validate()`, `toString()`), and `getState()` blanks them before persisting. They are nonetheless still *written* today — `syncFlatFieldsFromProfiles` and `syncFromActiveServer` both re-mirror the active server on a switch, and the legacy list beside them is still dual-written from the published profiles — which is the half of ADR-0049 that is unfinished, tracked in #206. Do not add a reader, and do not add sync code to keep a new field mirrored: the mirror is what that issue deletes. `EnvironmentSwitcher` (tool-window header) switches the active environment without opening Settings.
