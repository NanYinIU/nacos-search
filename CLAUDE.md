...
- `ConfigKeyExtractor` — key + `KeyLocation` extraction from configuration content, read under an explicitly supplied **运行时格式** (`RuntimeConfigFormat`). It derives no format of its own: the same body is a different key space under different rules, and only the caller knows which rules the runtime will apply. Its result is a closed set (`KeyExtraction`) — a parse, or a refusal carrying its reason — because a caller that flattens the two turns "we do not parse this format" into "this key is absent". That is the same invariant as the one above about data ids: only a successful parse under the 运行时格式 may prove a key absent, so do not add a `keysOrEmpty()` to it.
  ...
  Both share one `PathSeg` and one key renderer, so a leaf is addressed identically whichever format it came from — the bracket form (`list[0].n`) and the dot form (`list.0.n`), so either placeholder style resolves. A mapping key is held as **the text the runtime's flattening sees**, and the join is that runtime's own textual test: a key spelled `[…]` — whether the plugin bracketed it because its resolved value was not a string, or the user simply wrote `"[0]"` — joins without a separator and loses the dot spelling for the whole key, since a half-aliased `a[0].0` is a key nothing resolves either. The test is `startsWith("[")` and nothing cleverer: `[abc]` and an unclosed `[` take it too, and narrowing it to something index-shaped would part company with the runtime. Holding the spelling rather than the two causes that produce it is also what makes `0:` and `"[0]":` **one** entry, which is what the runtime makes of them — one map key, the later declaration winning, subtree and all. Both loaders apply that join identically, and the corpus asserts each against its own loader rather than assuming it.

  A mapping's entries therefore travel under **two** identities, and `entriesOf` keeps them apart because they disagree. snakeyaml resolves `<<` on the **constructed key** (`0:` is an `Integer`, `"0":` a `String`, so neither discards the other), and Spring's flattening then keys on the **text** (`0:` and `"[0]":` both spell `[0]`, so they collapse). The ordinary merge rule — a mapping's own key beats a merged one wherever the `<<` sits — belongs to the first level only. Where two entries collide at the *second*, the later one wins instead, so moving the `<<` line changes the answer. Doing both levels in one pass gets that winner backwards. What the first level actually does is about **slots**, not about own-versus-merged: snakeyaml replaces an overridden entry at the index it already holds, so an own key that overrides a merged one inherits the `<<`'s position rather than taking its own — which is what a `LinkedHashMap` keyed on the constructed key does for free, and why `mergeResolved` needs no notion of which entries are the mapping's own. The tests pin every ordering against the others so no one rule can absorb another. A container **with contents** contributes no key of its own: a runtime property source flattens to leaves, and `${list}` resolves against none of them. An **empty** container is where the two formats legitimately part company, and each follows the loader that reads it (ADR-0055): Spring Boot's YAML flattening terminates on one, so `list: []` contributes `list`, while Spring Cloud Alibaba's JSON loader drops it, so `{"list":[]}` contributes nothing. Do not make the two agree — the differential oracle holds both directions and the build will say so.
  - `keysUnderRuntimeFormat` is what every caller asks. It is still half a transitional bridge (#166): the format is now decided correctly, but the callers still read a refusal as "no keys", and a terminal resolution state replaces that later so a format the plugin can never parse stops requesting background namespace refreshes. It is one function so that change is one edit.
...
- `RuntimeFormatDecision` — the **运行时格式** decision table, and the only thing that decides which `RuntimeConfigFormat` a configuration is read under. It reads the data id suffix, case-folded as both runtimes fold it, and nothing else; the configuration's **声明格式** (`NacosConfiguration.declaredFormat()`, display-only — see **UI Layer**) participates in no row, because no runtime reads that field (ADR-0055). Its one deliberate departure from the runtime is the last row: an unrecognized suffix is `UNDETERMINED` and extracts nothing, where the runtime would fall back to properties — reading a YAML body under properties rules yields a well-formed `name` that a real `${name}` collides with, so following the runtime there manufactures false navigation rather than harmless noise. Do not "fix" it back, and do not add a second departure: a refusal the runtime would not make costs navigation that would have worked. It is its own unit with its own test so a row can change without silently changing another, and so the table is verified without a configuration body. The reference-site declaration (`@NacosPropertySource(type = ...)`) takes a row above the suffix rows when it lands; it needs a code context, so it will be a per-reference override rather than something the 派生 key 索引 can hold.
