...
- `ConfigKeyExtractor` — key + `KeyLocation` extraction from configuration content, read under an explicitly supplied **运行时格式** (`RuntimeConfigFormat`). It derives no format of its own: the same body is a different key space under different rules, and only the caller knows which rules the runtime will apply. Its result is a closed set (`KeyExtraction`) — a parse, or a refusal carrying its reason — because a caller that flattens the two turns "we do not parse this format" into "this key is absent". That is the same invariant as the one above about data ids: only a successful parse under the 运行时格式 may prove a key absent, so do not add a `keysOrEmpty()` to it. JSON is parsed by the Gson `JsonReader` the plugin already ships (`implementation("com.google.code.gson:gson")` — a declared dependency bundled into the distribution, not a platform library); properties are parsed by `java.util.Properties`, the loader both runtimes use, so the plugin and the runtime cannot disagree about properties syntax. Following that loader means a value containing whitespace followed by a hash keeps its full text — it has no inline comments — and that is deliberate, not an oversight to fix back. Neither parser supplies a line number, so for both the key set is authoritative and the line comes from a separate, permissive locator that reports `LINE_NOT_FOUND` rather than a wrong line, and a positioning defect can never corrupt the key space. Neither locator re-implements its parser's *decoding*: JSON's consumes key positions in document order, and properties' reads the name where the natural line spells it and hands an escaped spelling back to the loader. The properties locator does re-implement enough of the loader's *line* rules to recognize a comment and a continuation, which is safe only because it can never widen the key space — a name it reports that the loader did not produce is dropped. Neither line of a continuation may name a key: the second is the tail of a value however much it looks like an assignment, and the first names one only where a separator on it ends the name.
  - `keysUnderDeclaredType` is the transitional bridge (#166): the callers still derive the format from `NacosConfiguration.getConfigType()` and still read a refusal as "no keys". Both are known-wrong and land separately — the 运行时格式 decision table replaces the derivation, and a terminal resolution state replaces the refusal so a format the plugin can never parse stops requesting background namespace refreshes. It is one function so that change is one edit.
