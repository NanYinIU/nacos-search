...
Both declarations carry the `@CacheWriteAccess` opt-in marker, so naming either one without `@OptIn(CacheWriteAccess::class)` is a **compile error** (ADR-0051). Opting in belongs to components that perform remote operations and therefore hold an observation sequence: `CacheServiceOperationCache`, `NamespaceIndexCoordinator`, `NavigationDetailPrefetchService`, `MissingDetailRecorder`, `NacosApiService.clearCache`, and the cache module's own tests. It does not belong in `ui/` or `psi/` — those layers read the cache and never write to it. A UI component that needs a mutation asks the operation layer for it (the detail panel's authoritative not-found goes through `MissingDetailRecorder`); it does not opt in.
