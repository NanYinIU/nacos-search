    ...
    patchPluginXml {
       ...
       changeNotes.set("""
            ...
            <ul>
                ...
                <li><b>Navigation</b>: a declared <code>@NacosPropertySource</code> dataId is a hard filter. Hits under that dataId resolve as before; when the key is absent from the declared body (or the dataId itself is proven absent), the placeholder is unresolved with no gutter icon — never soft-falls to another cached configuration such as a sibling properties file. A dataId listed by a fresh authoritative Namespace index but not yet in the detail cache remains undecidable (hollow marker; click lazy-loads that dataId). When absence cannot be proven, resolution still does not substitute another dataId (issue #192 hard-filter).</li>
