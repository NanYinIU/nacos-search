...
/**
 * The operation layer's record of an authoritative not-found. The subject is
 * which observation sequence the delete carries: the one the read that proved
 * the configuration gone took, never a fresh one (issue #65 / ADR-0047).
 */
class MissingDetailRecorderTest {
    @get:Rule
    ...
    fun `an authoritative not-found deletes the cached detail`() = runBlocking {
        ...
        MissingDetailRecorder(cacheService)
